...
export default function MilestonesTab({
  ...
  const [showVersionHistoryId, setShowVersionHistoryId] = useState<
    string | null
  >(null);
  ...
  const [attachmentComments, setAttachmentComments] = useState<
    Record<string, AttachmentComment[]>
  >({});
  const [showCommentsAttachmentId, setShowCommentsAttachmentId] = useState<
    string | null
  >(null);
  const [loadingCommentsForAttachmentId, setLoadingCommentsForAttachmentId] =
    useState<string | null>(null);
  const [newCommentText, setNewCommentText] = useState<Record<string, string>>(
    {},
  );
  const [
    submittingCommentForAttachmentId,
    setSubmittingCommentForAttachmentId,
  ] = useState<string | null>(null);
  const [commentErrorByAttachmentId, setCommentErrorByAttachmentId] = useState<
  ...
  const confirmStatusChange = async () => {
    ...
    try {
      ...
      } else if (status === "no_submission") {
        // No Submission maps to draft status (same DB state)
  ...
  const handleToggleResolved = async (
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to toggle resolved",
      );
    }
  };
  ...
  const toggleAttachmentComments = async (attachmentId: string) => {
    const next =
      showCommentsAttachmentId === attachmentId ? null : attachmentId;
    ...
    return (
      ...
      >
        ...
        <MilestoneCard
          ...
          onStatusChange={
            isAdmin
              ? (status) => handleStatusChange(milestone.id, status)
              : undefined
          }
          isAdmin={isAdmin}
          onDocumentSubmittedChange={
            isAdmin && !milestone.is_public
              ? async (milestoneId, submitted) => {
                  ...
                  // Persist to database
                  await updateMilestoneDocumentSubmitted(
                    milestoneId,
                    submitted,
                  );
          ...
          publicProgress={
            milestone.is_public ? milestone.public_progress : undefined
          }
        />
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg border border-l-4 border-slate-200 border-l-un-blue bg-slate-50/30">
                  <div className="flex items-center gap-2 border-b border-slate-200 bg-white/80 px-3 py-2">
                    ...
                    {(() => {
                      const teamTop = updates.filter(
                        (u) => !u.reply_to && !u.is_legal && !u.is_internal,
                      );
                    ...
                    {updates.filter(
                      ...
                      <div className="space-y-2">
                        {updates
                          ...
                          .map((update) => {
                            const replies = updates.filter(
                              (r) => r.reply_to === update.id,
                            );
                                  ...
                                  <div className="p-3">
                                    <div className="mb-2 flex items-start justify-between gap-2">
                                      ...
                                      <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                                        <button
                                          onClick={() =>
                                            startReply(milestone.id, update.id)
                                          }
                                    ...
                                    <div className="border-t border-slate-200 bg-slate-50/50 px-3 py-2">
                                      <div className="space-y-2">
                                        {replies.map((reply) => (
                                          <div
                                            key={reply.id}
                                            className="group/reply relative flex gap-2 rounded-md bg-white p-2 hover:bg-slate-50"
                                          >
                ...
                {!milestone.is_public && isAdmin && (
                  <div className="rounded-lg border border-l-4 border-violet-200 border-l-violet-500 bg-violet-50/20">
                    <div className="flex items-center gap-2 border-b border-violet-200 bg-white/80 px-3 py-2">
                      <MessageSquare className="h-4 w-4 text-violet-600" />
                      <span className="text-xs font-semibold tracking-wide text-violet-800/90 uppercase">
                        Internal comments
                      </span>
                      ...
                      {(() => {
                        const internalTop = updates.filter(
                          (u) => !u.reply_to && !u.is_legal && u.is_internal,
                        );
                      ...
                      {updates.filter(
                        ...
                        <div className="space-y-2">
                          {updates
                            .filter(
                              (u) =>
                                !u.reply_to && !u.is_legal && u.is_internal,
                            )
                            .map((update) => {
                              const replies = updates.filter(
                                (r) => r.reply_to === update.id,
                                ...
                                <div key={update.id} className="space-y-2">
                                  <div
                                    className={`group relative rounded-lg border transition-all ${
                                      update.is_resolved
                                        ? "border-green-200 bg-green-50/30"
                                        : "border-violet-200 bg-white hover:border-violet-400/50 hover:shadow-sm"
                                    }`}
                                  >
                                    <div className="p-3">
                                      <div className="mb-2 flex items-start justify-between gap-2">
                                      ...
                                      <div className="border-t border-violet-100 bg-violet-50/30 px-3 py-2">
                                        <div className="space-y-2">
                                          {replies.map((reply) => (
                                            <div
                                              key={reply.id}
                                              className="group/reply relative flex gap-2 rounded-md bg-white p-2 hover:bg-violet-50/50"
                                            >
                ...
                {milestone.is_public && isAdmin && (
                  <div className="rounded-lg border border-l-4 border-amber-200 border-l-amber-500 bg-amber-50/20">
                    <div className="flex items-center gap-2 border-b border-amber-200 bg-white/80 px-3 py-2">
                      <Scale className="h-4 w-4 text-amber-600" />
                      <span className="text-xs font-semibold tracking-wide text-amber-800/90 uppercase">
                        Legal updates & comments
                      </span>
                      {(() => {
                        const legalTop = updates.filter(
                          (u) => !u.reply_to && u.is_legal,
                        );
                    ...
                    <div className="p-2">
                      {updates.filter((u) => !u.reply_to && u.is_legal).length >
                      0 ? (
                        <div className="space-y-2">
                          {updates
                            .filter((u) => !u.reply_to && u.is_legal)
                            .map((update) => {
                              const replies = updates.filter(
                                (r) => r.reply_to === update.id,
                              );
                                ...
                                <div key={update.id} className="space-y-2">
                                  <div
                                    className={`group relative rounded-lg border transition-all ${
                                      update.is_resolved
                                        ? "border-green-200 bg-green-50/30"
                                        : "border-amber-200 bg-white hover:border-amber-400/50 hover:shadow-sm"
                                    }`}
                                  >
                                    <div className="p-3">
                                      <div className="mb-2 flex items-start justify-between gap-2">
                                      ...
                                      <div className="border-t border-amber-100 bg-amber-50/30 px-3 py-2">
                                        <div className="space-y-2">
                                          {replies.map((reply) => (
                                            <div
                                              key={reply.id}
                                              className="group/reply relative flex gap-2 rounded-md bg-white p-2 hover:bg-amber-50/50"
                                            >
                            ...
                            <div className="flex shrink-0 gap-1">
                              {isAdmin && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    toggleAttachmentComments(att.id)
                                  }
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="flex gap-2">
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      handleAddAttachmentComment(att.id)
                                    }
                                    disabled={
                                      submittingCommentForAttachmentId ===
                                        att.id ||
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {confirmDialog.status === "draft" &&
                "Change this milestone to Draft? It will no longer be approved."}
              {confirmDialog.status === "no_submission" &&
              ...
              {confirmDialog.status === "needs_attention" &&
                "Mark this milestone as needing attention? This will notify the team to make changes."}
              {confirmDialog.status === "needs_ola_review" &&
                "Mark this milestone as needing OLA (Office of Legal Affairs) review?"}
              {confirmDialog.status === "reviewed_by_ola" &&
                "Mark this milestone as reviewed by OLA (Office of Legal Affairs)?"}
              {confirmDialog.status === "finalized" &&
                "Finalize this milestone? This marks it as complete."}
              {confirmDialog.status === "attention_to_timeline" &&
                "Mark this milestone as needing attention to timeline?"}
              {confirmDialog.status === "confirmation_needed" &&
                "Mark this milestone as needing confirmation?"}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
