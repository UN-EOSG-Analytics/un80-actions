...
UN80 Actions Dashboard — a static public Next.js site tracking UN80 Initiative work packages and action items. Data flows from Excel → Power Automate → GitHub → `public/data/actions.json` (daily refresh). The app is a **static export** (`output: 'export'` in `next.config.ts`), deployed to GitHub Pages with no server-side runtime.
