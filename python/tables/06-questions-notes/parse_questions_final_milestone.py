"""
Parse the "Questions on Final Milestone" column from the Action Plan Excel
and produce a structured JSON file with one question per row.

Uses the same approach as parse_notes_from_task_force and parse_questions_on_initial_milestone:
- Split each cell on double newline to get multiple segments.
- Optionally extract date from segment; if no date, question_date is null ("no date").
- Header is always "Unspecified" (applied at import time).

Output: data/processed/questions_on_final_milestone.json
  [
    { "ActionNo": 14, "question_date": "2025-01-21", "question": "..." },
    { "ActionNo": 14, "question_date": null, "question": "..." },
    ...
  ]
"""
...
QUESTIONS_COL_NAME = "Questions on Final Milestone"
...
def extract_date_and_body(segment: str) -> Tuple[Optional[str], str]:
    """Extract question_date (ISO) and body from a segment. If no date, returns (None, body)."""
...
def main() -> None:
    ...
    output_path = output_dir / "questions_on_final_milestone.json"
